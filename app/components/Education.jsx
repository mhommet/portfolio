      ...
      <div className="container mx-auto px-4">
        ...
        <div className="flex flex-col gap-8 w-full lg:w-3/4 mx-auto">
          {educations.map((edu, index) => (
            <div key={index} className="card bg-base-200 shadow-lg hover:shadow-xl transition-shadow border border-primary/20">
              <div className="card-body p-5">
                <div className="flex items-start gap-4">
                  <div className="bg-primary p-2 rounded-full shrink-0 mt-1">
                    <FaGraduationCap className="text-primary-content" size={20} />
                  </div>
                  <div className="flex-1">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                      <h3 className="card-title text-lg">{edu.degree}{edu.type && ` - ${edu.type}`}</h3>
                    </div>
                      ...
                      <ul className="space-y-1">
                        {edu.skills.map((skill, idx) => (
                          <li key={idx} className="flex items-start">
                            <FaCheck className="text-primary mt-1 mr-2 shrink-0" size={12} />
